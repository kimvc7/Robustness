...
class Model(object):
  def __init__(self, num_features):
    ...
    for k in range(num_classes):
      ...
      g_2k_pos1 = tf.boolean_mask(self.g_2_pos1, mask)
      g_2k_pos2 = tf.boolean_mask(self.g_2_pos2, mask)
      g_2k_neg1 = tf.boolean_mask(self.g_2_neg1, mask)
      g_2k_neg2 = tf.boolean_mask(self.g_2_neg2, mask)
      filt2_k = tf.boolean_mask(filt2, mask)
      W3_k = self.W3 - tf.gather(self.W3, [k], axis=1)

      g_3k_pos = tf.matmul(tf.nn.relu(g_2k_pos1 + self.b2), tf.nn.relu(W3_k)) - tf.matmul(tf.multiply(-g_2k_pos2 + self.b2, filt2_k), tf.nn.relu(-W3_k))
      g_3k_neg = tf.matmul(tf.nn.relu(g_2k_neg1 + self.b2), tf.nn.relu(W3_k)) - tf.matmul(tf.multiply(-g_2k_neg2 + self.b2, filt2_k), tf.nn.relu(-W3_k))
      g_3k_max = tf.maximum(g_3k_pos, g_3k_neg)
      g_3k = tf.nn.max_pool(tf.reshape(g_3k_max, [1, tf.shape(g_3k_max)[0], num_classes, 1]), [1, num_features, 1, 1], [1, num_features, 1, 1], "SAME") + tf.reshape(self.b3  - self.b3[k], [1, 1, num_classes, 1])
      

      robust_acc +=  tf.reduce_sum(tf.cast(tf.reduce_all(tf.less_equal(g_3k, tf.constant([0.0])), axis = 2), tf.float32))
      robust_objective +=  tf.reduce_sum( tf.reduce_logsumexp(g_3k, axis = 2))
    ...
    #robust_outputs = tf.transpose(tf.reshape(tf.vectorized_map(self._loss_fn, (self.x_input, self.y_input, self.z1, self.h1, self.h2)), (-1, 10)))
  ...
  def _loss_fn(self, args):
      ...
      filt = tf.sign(h1)
      filt2 = tf.sign(h2)
      W13 = self.W3 - tf.gather(self.W3, [y], axis=1)
      ...
      g_3k_pos = tf.matmul(tf.nn.relu(g2pos1 + self.b2), tf.nn.relu(W13)) - tf.matmul(tf.multiply(-g2pos2 + self.b2, filt2), tf.nn.relu(-W13))
      g_3k_neg = tf.matmul(tf.nn.relu(g2neg1 + self.b2), tf.nn.relu(W13)) - tf.matmul(tf.multiply(-g2neg2 + self.b2, filt2), tf.nn.relu(-W13))
      return tf.reduce_max( tf.maximum(g_3k_pos , g_3k_neg), axis = 0) + self.b3 - self.b3[y]
